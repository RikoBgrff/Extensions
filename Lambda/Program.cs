...
namespace Lambda
{
    ...
    class Program
    {
        ...
        static void Main(string[] args)
        {
            ...
            Console.WriteLine("7.Borcu, umumi borclarin orta borcunnan cox olan borclulari cixarmaq ve evvel familyaya gore sonra ise meblegin azalmagina gore sortirovka etmek");
            ...
            Console.WriteLine("9.Telefon nomresinde 8 olmayan borclularin yalniz familyasin,yashin ve umumi borcun meblegin cixarmaq");
            Console.WriteLine("10.borclulardan en coxu hansi ilde dogulubsa hemin ili cixartmaq");
            Console.WriteLine("11.Borcu en boyuk olan 5 borclunun siyahisini cixartmaq");
            Console.WriteLine("12.Butun borcu olanlarin borcunu cemleyib umumi borcu cixartmaq");
            Console.WriteLine("13.2ci dunya muharibesin gormush borclularin siyahisi cixartmaq");
            Console.WriteLine("14.Nomresinde tekrar reqemler olmayan borclularin ve onlarin borcunun meblegin cixartmaq");
            Console.WriteLine("15.Tesevvur edek ki,butun borclari olanlar bugunden etibaren her ay 500 azn pul odeyecekler. ");
            Console.WriteLine("Oz.ad gunune kimi borcun oduyub qurtara bilenlerin siyahisin cixartmaq");
            Console.WriteLine("16.Adindaki ve familyasindaki herflerden 'smile' sozunu yaza bileceyimiz borclularin siyahisini cixartmaq");

            Console.Write("\nYour Choice:"); string choice = Console.ReadLine();
            if (choice == "1")
            {
                var debtorsByEmail = debtors
                    .FindAll(d => d.Email.Contains("rhyta.com") || d.Email.Contains("dayrep.com"));
                debtorsByEmail
                    .ForEach(d => Console.WriteLine($"{d.FullName} - {d.Debt}"));
            }
            if (choice == "2")
            {
                var debtorsbyAge26 = debtors
                    .FindAll(d => d.BirthDay.Year > 1985 && d.BirthDay.Year < 1995);
                debtorsbyAge26.ForEach(d => Console.WriteLine($"{d.FullName} - {d.Debt}"));
            }
            if (choice == "3")
            {
                var debtorsbylowerthan5000 = debtors
                    .FindAll(d => d.Debt < 5000);
                debtorsbylowerthan5000.ForEach(d => Console.WriteLine($"{d.FullName} - {d.Debt}"));
            }
            if (choice == "4")
            {
            ...
            if (choice == "7")
            {
                var debtorsNotContains8 = debtors
                    .Where(d => !d.Phone.Contains('8'))
                    .ToList();

                debtorsNotContains8.ForEach(d =>
                {
                    var namee = d.FullName.Split(' ');

                    Console.WriteLine($"{namee[namee.Length - 1]} {DateTime.Now.Year - d.BirthDay.Year} {d.Phone} {d.Debt}");
                });
            }
            if (choice == "8")
            {
                Console.WriteLine("Could not write it");
            }
            if (choice == "9")
            {
                var years = debtors.Select(d => d.BirthDay.Year).ToList();

                var year = years.GetMostWantedData();

                Console.WriteLine(year);
            }
            if (choice == "10")
            {
                var best5Debtors = debtors
                    .OrderByDescending(d => d.Debt)
                    .Take(5)
                    .ToList();

                best5Debtors.ForEach(Console.WriteLine);
            }
            if (choice == "11")
            {
                var total = debtors
            ...
            if (choice == "12")
            {
                var debtorsWhoHadSeenWW2 = debtors
                    .Where(d => d.BirthDay.Year >= 1938 &&
            ...
            if (choice == "14")
            {
                var debtTillBirthday = 6000;

                Func<int, int, bool> Checker = (debt, month) => debt - (month * 500) <= 0;
                var debtors6 = debtors.Where(d =>
                {
                    if (d.Debt <= debtTillBirthday)
                    {
                        ...
                        {
                            if (Checker(d.Debt, (birthMonth - nowMonth)))
                                return true;
                        }
                        else
                        {
                            if (Checker(d.Debt, ((12 + birthMonth) - nowMonth)))
                                return true;
                        }
                    }
                    return false;
            ...
            if (choice == "15")
            {
                var debtorsWhoCanSmile = debtors.Where(d =>
                {
                ...
                debtorsWhoCanSmile.ForEach(Console.WriteLine);
            }
        }
    }
}
...
7) Borcu, umumi borclarin orta borcunnan cox olan borclulari cixarmaq ve evvel familyaya gore sonra ise meblegin azalmagina gore sortirovka etmek
...
10)borclulardan en coxu hansi ilde dogulubsa hemin ili cixartmaq
...
15)Tesevvur edek ki,butun borclari olanlar bugunden etibaren her ay 500 azn pul odeyecekler. 
